...
**See also:** [PILOT-MANUAL-TEST-CHECKLIST.md](PILOT-MANUAL-TEST-CHECKLIST.md) — broader **pre-pilot manual** checklist (devices, offline, session, email, sign-off). [TESTING-AND-QUALITY-WORKFLOW.md](TESTING-AND-QUALITY-WORKFLOW.md) — how roadmap completion updates this file, automation, pilot list, and [TEST-RESULTS-LOG.md](TEST-RESULTS-LOG.md).
...
### Path 7: Session refresh (long-shift scanner)

**Automated:** `GET /api/me/profile` remains the lightweight probe (see `apiService.pingSession`).

**Manual (2026-04-04+):** On app scanner (`CheckInScanner`, standalone): after backgrounding the tab or waiting several minutes, returning to the tab should re-validate session (Clerk `getToken({ skipCache: true })` + probe). If the amber banner appears, **Refresh session** should attempt recovery before a full re-login; **Sign in** / **Continue setup** remain the fallback.

### Path 8: Bulk QR ZIP (admin)

1. Open Event Workspace with an event that has multiple attendees (organizer).
2. Select two or more rows → **QR ZIP**.
3. Verify a `.zip` downloads; PNGs scan or match single-attendee QR for same guest (export does not rotate tokens while still valid — see `getQRPayloadForExport`).
