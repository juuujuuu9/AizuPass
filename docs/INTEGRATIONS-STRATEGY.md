...
| **Eventbrite (organizer)** | Pull guestlist into an event | Admin → **Integrations** (`/admin/events/integrations`): Eventbrite event ID + [private token](https://www.eventbrite.com/platform/docs/authentication); `POST /api/integrations/eventbrite/sync`; idempotency via `microsite_entry_id` prefix `eb:{attendeeId}`; optional “save credentials” stores token in `events.settings.eventbrite` (stripped from all `rowToEvent` responses). |
